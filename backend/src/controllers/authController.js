...
export const login = asyncHandler(async (req, res) => {
  ...
  // ── No account found ──────────────────────────────────────────────────────
  // We use a distinct code (USER_NOT_FOUND) so our *own* frontend can show a
  // helpful "create an account" prompt, but we keep the human-readable message
  // intentionally vague so it's still safe against casual enumeration attempts.
  if (!user) {
    throw new APIError(401, "Invalid email or password", "USER_NOT_FOUND");
  }
  ...
  if (!isPasswordValid) {
    // Distinct code so the frontend knows to show the generic "wrong password" message
 ...
 *
 * FIX: Returns a machine-readable `code` field so the frontend can open the
 * NoAccountModal, while HTTP status stays 200 in both cases so network-level
 * enumeration is still impossible.
 */
export const forgotPassword = asyncHandler(async (req, res) => {
  ...
  // ── No account found ──────────────────────────────────────────────────────
  // HTTP 200 is intentional: prevents enumeration at the network layer.
  // The `code: "USER_NOT_FOUND"` field is how OUR frontend detects this case.
  // A third-party observer only sees "200 OK" either way.
  if (!user) {
